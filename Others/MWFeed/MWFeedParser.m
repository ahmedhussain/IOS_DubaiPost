    ...
    @autoreleasepool {
        ...
        if (!processed) {
            if (((feedType == FeedTypeRSS || feedType == FeedTypeRSS1) && [qName isEqualToString:@"item"]) ||
                ...
                if (!item.media && (item.summary || item.content)){
                    NSString *imgurl = nil;
                    ...
                    if (![theScanner isAtEnd]) {
                        ...
                        [theScanner scanUpToCharactersFromSet:charset intoString:&imgurl];
                        // "url" now contains the URL of the img
                     /*   imgurl =[imgurl stringByAddingPercentEscapesUsingEncoding:NSASCIIStringEncoding];
                        NSURL *imageUrl =[NSURL URLWithString:imgurl];
                        NSData *imageData = [NSData dataWithContentsOfURL:imageUrl];
                        UIImage *image = [[UIImage alloc]initWithData:imageData];
                        if (image.size.height > 3 && image.size.width > 3){
                            item.media = imgurl;
                        }
                      */
                    }
